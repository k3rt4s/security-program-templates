...
- A postmortem missing-telemetry fault is the strongest evidence that a Coverage confidence claim was wrong. When the source already exists, record it as a Coverage confidence downgrade rather than a new row.
...
- The [control evidence acceptance register](../compliance/control-evidence-acceptance-register.md) answers a different evidence question: whether an item supports a control assertion for a stated scope and period. A retrieval test recorded here can be accepted there as evidence that a logging or response control operated, while this register remains the source for what incident fact the underlying evidence can prove.
