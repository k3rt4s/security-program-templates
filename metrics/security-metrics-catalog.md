...
| Classes eliminated                 | Root causes fixed, e.g. a base image, not individual findings closed     | Count findings closed instead, which is the flattering number      | Nothing, and that is why it is the one worth adding. It is the only measure that tracks arrival rather than closure                     |
