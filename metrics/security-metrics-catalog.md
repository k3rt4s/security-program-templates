...
A borrowed population loss figure is the numerator-side twin of a self-referential denominator: it looks like evidence, survives publishing the source, and still describes the wrong population. The [board update's cost-of-doing-nothing guidance](../board-reporting/quarterly-security-update-template.md#section-8-asks) applies the same test to asks framed as risk reduction or cost avoidance.
